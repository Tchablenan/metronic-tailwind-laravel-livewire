...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-5 pb-8">
        <div class="flex flex-col gap-2">
            ...
            <p class="text-sm text-gray-600 font-medium">
                @if($user->role === 'doctor')
                    Valider ou rejeter les demandes payées
                @else
        ...
        {{-- 🆕 BOUTON NOUVELLE DEMANDE (Secrétaire uniquement) --}}
    ...
    <!-- ========================================
         ...
         ======================================== -->
    @if($user->role === 'doctor')
    <!-- Stats Médecin: À valider, Converties, Rejetées, Urgentes, Montant -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <!-- Stat: À Valider -->
        <div class="bg-white rounded-xl shadow-md p-5 border-l-4 border-yellow-500">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-xs font-medium text-gray-600 uppercase tracking-wide">À Valider</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $statistics['to_validate'] }}</p>
                </div>
                <div class="text-4xl text-yellow-400 opacity-20">
                    <i class="ki-filled ki-hourglass"></i>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-5 border-l-4 border-green-500">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-xs font-medium text-gray-600 uppercase tracking-wide">Converties</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $statistics['converted'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-5 border-l-4 border-orange-500">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-xs font-medium text-gray-600 uppercase tracking-wide">Urgentes</p>
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $statistics['urgent'] }}</p>
                </div>
                <div class="text-4xl text-orange-400 opacity-20">
    ...
    @else
    <!-- Stats Secrétaire: En Attente, En Traitement, Complétées, Montant, % Données Médicales -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <!-- Stat: En Attente -->
        <div class="bg-white rounded-xl shadow-md p-5 border-l-4 border-yellow-500">
            <div class="flex items-center justify-between">
    ...
    <div class="bg-white rounded-xl shadow-md mb-6">
        ...
        <form method="GET" action="{{ route('service-requests.index') }}" id="filterForm" class="hidden p-6 bg-gray-50">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                ...
                <!-- Statut de Paiement (Secrétaire uniquement) -->
                @if($user->role === 'secretary')
                <div>
                ...
                <!-- Checkbox: Assurée (Secrétaire uniquement) -->
                @if($user->role === 'secretary')
                ...
                @else
                <!-- Checkbox: Données Médicales (Médecin également) -->
                <div class="flex items-end">
                    <label class="flex items-center gap-2.5 p-2.5 bg-white border border-gray-300 rounded-lg hover:border-blue-500 cursor-pointer transition-all">
                        <input type="checkbox" name="has_medical_data" value="1" {{ $hasMedicalData ? 'checked' : '' }}
                               class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                        ...
                        <!-- Indicateurs: Données Médicales, Assurance, Fichier Examen (Médecin + Secrétaire) -->
                        <td class="px-4 py-3">

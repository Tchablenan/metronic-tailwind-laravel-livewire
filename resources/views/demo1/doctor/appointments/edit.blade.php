                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <select name="nurse_id" id="nurse_id"
                            ...
                            @foreach ($nurses as $nurse)
                                <option value="{{ $nurse->id }}"
                                    {{ old('nurse_id', $appointment->nurse_id) == $nurse->id ? 'selected' : '' }}>
                                    {{ $nurse->full_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            @foreach ($types as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('type', $appointment->type) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="time" name="appointment_time" id="appointment_time"
                            value="{{ old('appointment_time', $appointment->formatted_time) }}" required
                    ...
                    <div>
                        ...
                        <select name="duration" id="duration" required
                            ...
                            <option value="15" {{ old('duration', $appointment->duration) == 15 ? 'selected' : '' }}>15
                                minutes</option>
                            <option value="30" {{ old('duration', $appointment->duration) == 30 ? 'selected' : '' }}>30
                                minutes</option>
                            <option value="45" {{ old('duration', $appointment->duration) == 45 ? 'selected' : '' }}>45
                                minutes</option>
                            <option value="60" {{ old('duration', $appointment->duration) == 60 ? 'selected' : '' }}>1
                                heure</option>
                            <option value="90" {{ old('duration', $appointment->duration) == 90 ? 'selected' : '' }}>
                                1h30</option>
                            <option value="120" {{ old('duration', $appointment->duration) == 120 ? 'selected' : '' }}>2
                                heures</option>
                        </select>
                        @error('duration')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="location" id="location" required
                            ...
                            @foreach ($locations as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('location', $appointment->location) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- ⬇️⬇️⬇️ NOUVEAU : Statut (réservé au médecin chef) ⬇️⬇️⬇️ -->
                    @if (auth()->user()->role === 'doctor')
                        <div>
                            ...
                            <select name="status" id="status" required
                                class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white @error('status') border-red-500 @enderror">
                                <option value="scheduled"
                                    {{ old('status', $appointment->status) == 'scheduled' ? 'selected' : '' }}>Prévu
                                </option>
                                <option value="confirmed"
                                    {{ old('status', $appointment->status) == 'confirmed' ? 'selected' : '' }}>Confirmé
                                </option>
                                <option value="in_progress"
                                    {{ old('status', $appointment->status) == 'in_progress' ? 'selected' : '' }}>En cours
                                </option>
                                <option value="completed"
                                    {{ old('status', $appointment->status) == 'completed' ? 'selected' : '' }}>Terminé
                                </option>
                                <option value="cancelled"
                                    {{ old('status', $appointment->status) == 'cancelled' ? 'selected' : '' }}>Annulé
                                </option>
                                <option value="no_show"
                                    {{ old('status', $appointment->status) == 'no_show' ? 'selected' : '' }}>Absent
                                </option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="price" id="price"
                            value="{{ old('price', $appointment->price) }}" min="0" step="100"
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="3" placeholder="Notes privées pour le personnel médical..."
                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none @error('notes') border-red-500 @enderror">{{ old('notes', $appointment->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>

                    <!-- ✅ Notes pour le patient (sur toute la largeur) -->
                    <div class="lg:col-span-2">
                    ...
                    <!-- ⬇️⬇️⬇️ Raison d'annulation (si le statut est annulé) ⬇️⬇️⬇️ -->
                    @if (auth()->user()->role === 'doctor')
                        <div class="lg:col-span-2" id="cancellation-reason-field"
                            style="display: {{ old('status', $appointment->status) == 'cancelled' ? 'block' : 'none' }};">
                            ...
                            <textarea name="cancellation_reason" id="cancellation_reason" rows="2"
                                ...
                                class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none @error('cancellation_reason') border-red-500 @enderror">{{ old('cancellation_reason', $appointment->cancellation_reason) }}</textarea>
...
@push('scripts')
    ...
    <script>
        // Afficher/masquer le champ raison d'annulation selon le statut
        @if (auth()->user()->role === 'doctor')
            document.addEventListener('DOMContentLoaded', function() {
                const statusSelect = document.getElementById('status');
                const cancellationField = document.getElementById('cancellation-reason-field');

                if (statusSelect && cancellationField) {

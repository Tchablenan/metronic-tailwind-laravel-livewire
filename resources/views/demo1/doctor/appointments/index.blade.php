...
@section('content')
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <!-- Card principal -->
        <div class="bg-white rounded-xl shadow-md overflow-y-visible">

            <!-- Messages flash -->
            @if (session('success'))
                <div
                    class="m-4 px-4 py-3.5 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 text-green-800 rounded-lg shadow-sm">
            ...
            @if (session('error'))
                <div
                    class="m-4 px-4 py-3.5 bg-gradient-to-r from-red-50 to-rose-50 border border-red-200 text-red-800 rounded-lg shadow-sm">
                    <div class="flex items-center justify-between">
                        <span class="flex items-center gap-2.5 font-medium">
            ...
            <div class="p-5 bg-gray-50/50 border-b border-gray-200">
                <form method="GET" action="{{ route('appointments.index') }}"
                    ...
                    <div class="w-80">
                        <div
                            class="flex items-center gap-2 px-3 py-2 border border-gray-300 rounded-lg bg-white focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 focus-within:shadow-sm transition-all">
                    ...
                    @if (request('search') || request('date') || request('status') || request('type') || request('doctor_id'))
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b" style="border-color: #f0f0f0;">
                            ...
                            <th
                                class="w-20 px-4 py-4 text-center font-medium text-gray-600 text-xs uppercase tracking-wide">
                                Actions
                            </th>
                        </tr>
                    </thead>
                            ...
                            <tr class="border-b hover:bg-gray-50/50 transition-colors" style="border-color: #f5f5f5;">
                                ...
                                <td class="px-4 py-4 text-center">
                                    <div class="relative inline-block" x-data="{ open: false }">
                                        ...
                                        <!-- ⬇️ RETIRE x-cloak et ajoute x-transition ⬇️ -->
                                        <div x-show="open" @click.away="open = false"
                                            ...
                                            @can('delete', $appointment)
                                                ...
                                                <form action="{{ route('appointments.destroy', $appointment) }}"
                                                    ...
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="flex items-center gap-3 px-4 py-2 text-sm text-red-600 hover:bg-red-50 w-full text-left transition-colors btn-delete-appointment"
                                                        data-appointment-id="{{ $appointment->id }}">
                            ...
                            <tr>
                                <td colspan="7" class="px-4 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if (request()->hasAny(['search', 'date', 'status', 'type', 'doctor_id']))
            ...
            @if ($appointments->hasPages())
                <div
                    class="px-5 py-4 bg-gray-50/50 border-t border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div class="text-sm text-gray-600 font-medium">
                        Affichage de <span class="font-bold text-gray-900">{{ $appointments->firstItem() }}</span> à <span
                            class="font-bold text-gray-900">{{ $appointments->lastItem() }}</span> sur <span
                            class="font-bold text-gray-900">{{ $appointments->total() }}</span> rendez-vous
                    </div>
                    <div>
                        {{ $appointments->links() }}
                    </div>
